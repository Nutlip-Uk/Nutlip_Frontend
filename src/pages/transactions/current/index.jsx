...
const Current = () => {
  ...
  useEffect(() => {
    const fetchAcceptedOffer = async () => {
      ...
      try {
        const response = await fetch(`https://nutlip-backend-wdsi.onrender.com/api/offer/getofferssent/${userId}`);
        ...
        if (Array.isArray(data.offers)) {
          ...
          const fetchAdditionalData = acceptedOffers.map(async (offer) => {
            const additionalResponse = await fetch(`https://nutlip-backend-wdsi.onrender.com/api/apartments/getapartment/${offer.apartmentId}`);
