...
export const AddConveyancer = ({ userType, transaction, id, userInformation, transactionContent }) => {
    ...
    const fetchConveyancerDetails = async (conveyancerID, type) => {
        ...
        try {
            const response = await fetch(`aws url for now, http://ec2-13-60-41-27.eu-north-1.compute.amazonaws.comapi/users/${conveyancerID}`);
    ...
    const addSellerConveyancer = async (conveyancerID) => {
        try {
            const response = await fetch("aws url for now, http://ec2-13-60-41-27.eu-north-1.compute.amazonaws.comapi/transaction/transaction_selleraddconveyancer_03", {
                method: "PUT",
                headers: {
    ...
    const addBuyerConveyancer = async (conveyancerID) => {
        try {
            const response = await fetch("aws url for now, http://ec2-13-60-41-27.eu-north-1.compute.amazonaws.comapi/transaction/transaction_buyerconveyancer_04", {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                },
